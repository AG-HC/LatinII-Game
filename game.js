...
let currentTrial = 1;
// Random encounters
let sawBandits = false, sawFate = false, sawLetter = false;

const letters = ['a', 'b', 'c', 'd'];
// Go wild
const dmgText = ["Ouch", "Owie", "That hurt :("];

// Roll for when guaranteed encounters appear
const enFates = Math.floor(Math.random()*2)+8;
const enLetter = Math.floor(Math.random()*2)+10;
console.log(`Encouters after trials: ${enFates}, ${enLetter}`);
...
function encounter() {
  ...
  if (currentTrial>3 && sawBandits == false) {
    const enBandits = Math.floor(Math.random()*9);
    if (enBandits < 5) {
      console.log("Bandit time");
      sawBandits = true;
      textElement.innerText+="While on your way back to Eurystheus, you see someone further up the road...\n\n";
      startTrial(en1);
    } else {
      console.log(`No bandits :( (${enBandits})`);
    }
  }
...
// Twine's Change-Game-Language-inator
const title = document.getElementById('Title');
const desc = document.getElementById('Desc');
const topBar = document.getElementById('Top');
const themes = document.getElementById('themeLink');
...
function changeLanguage(lang) {
  ...
  switch (lang) {
    ...
    case 'bin':
      ...
      healthElement.innerText = "01001000 01100101 01100001 01101100 01110100 01101000 00111010 00100000 00110101";
      break;
    case 'shk':
